...
\author{ENG EC 503\\Karanraj Chauhan, Berk Gur, Vijay Thakkar}

% \begin{abstract}
% %% Text of abstract
% In this project we compare performances of various unsupervised clustering algorithms. The baseline algorithm to be used is the vanilla k-Means, and the algorithms to be compared are k-Means++, DP-Means, DBSCAN, and OPTICS. Additionally, if time permits, we would like to implement an approximation algorithm to compute the assignments of data points after finding cluster centroids.
% \end{abstract}
...
Unsupervised learning is the type of machine learning in which labels are determined from the data in which the corresponding labels are not available. The most common unsupervised learning method is cluster analysis, which is used for exploratory data analysis to find hidden patterns or grouping in data. Clustering algorithms group data into clusters based on some similarity measure (e.g. Euclidean distance, Jaccard similarity, cosine similarity, etc.) defined in some space.

One of the most common clustering algorithms is k-Means clustering. k-Means clustering maps input data into k distinct clusters based on Euclidean distance to the centroid of each cluster. It is often computationally faster than hierarchical clustering as well as scalable for large data, for small values of k. Furthermore, it produces tighter clusters than hierarchical clustering, especially if the clusters are globular. However, its correctness and accuracy depends on a good choice for value of k, which varies from dataset to dataset and can be challenging to predict. Moreover, convergence of the algorithm is sensitive to initial selection of cluster centroids.

One way to improve k-means is by using better initializations for cluster centroids. This is the idea behind k-means++. The centroids are chosen such that the distance of each point from the nearest centroid is maximized.

Another improvement over the vanilla k-means is explored in DP-means. This algorithm does not take any input for k. Instead, it determines the value of k at runtime. Its objective function has a term to impose penalty on large values of k (structural risk minimization). But we have to specify the constant for penalizing on the value of k. This constant is dataset dependent and may be difficult to determine.

Another popular method for clustering is Density-based spatial clustering of applications with noise (DBSCAN). This algorithm has an altogether different approach than k-means. Here, clusters are created based on the density of data in mapping space.


% Means Advantages : 1) If variables are huge, then K-Means most of the times computationally faster than hierarchical clustering, if we keep k smalls. 2) K-Means produce tighter clusters than hierarchical clustering, especially if the clusters are globular. K-Means Disadvantages : 1) Difficult to predict K-Value.Feb 14, 2013
